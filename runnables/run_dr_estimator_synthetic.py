...
def main(cfg: DictConfig):
    ...
    if cfg.exp.logging:
        ...
        mlflow.set_experiment("Synthetic_DR")
        ...
        with mlflow.start_run():
            ...
            ############################################################################################################
            # 4) Evaluate
            dr_predictions = torch.softmax(dr_model.policy(torch.tensor(generator_test.X).float()), dim=1).detach().numpy()
            dr_value = generator_test.evaluate_policy(dr_predictions)
            randomized_value = generator_test.evaluate_policy(np.ones(dr_predictions.shape) / dr_predictions.shape[1])
